...
public class JobViewTest {
	...
	public void should_know_the_url_of_the_last_build() {
		...
		view = JobView.of(a(job().whereTheLast(build().numberIs(22))), locatedAt("job/project-name"));
	...
	public void progress_of_a_nearly_finished_job_should_be_100() throws Exception {
		...
		view = JobView.of(a(job().whereTheLast(build().isStillBuilding().startedAt("12:00:00").andUsuallyTakes(0))),
	...
	public void progress_of_a_job_thats_taking_longer_than_expected_should_be_100() throws Exception {
		...
		view = JobView.of(a(job().whereTheLast(build().isStillBuilding().startedAt("12:00:00").andUsuallyTakes(5))),
	...
	public void should_calculate_the_progress_of_a_running_job() throws Exception {
		...
		view = JobView.of(a(job().whereTheLast(build().isStillBuilding().startedAt("13:10:00").andUsuallyTakes(5))),
	...
	public void should_know_how_long_a_build_has_been_running_for() throws Exception {
		...
		String startTime = "13:10:00", sixSecondsLater = "13:10:06", twoAndHalfMinutesLater = "13:12:30", anHourAndHalfLater = "14:40:00";
		Date currentTime = assumeThatCurrentTime().is(startTime);

		view = JobView.of(a(job().whereTheLast(build().startedAt(startTime).isStillBuilding())), currentTime);
	...
	public void should_describe_the_job_as_running_if_it_is_running() {
		...
		List<JobView> views = asFollows(JobView.of(a(job().whereTheLast(build().hasntStartedYet()))),
	...
	public void should_describe_the_job_as_running_and_successful_if_it_is_running_and_the_previous_build_succeeded() {
		...
		List<JobView> views = asFollows(JobView.of(a(job().whereTheLast(build().hasntStartedYet()).andThePrevious(
				build().finishedWith(SUCCESS)))),

		JobView.of(a(job().whereTheLast(build().isStillBuilding()).andThePrevious(build().finishedWith(SUCCESS)))),

				JobView.of(a(job().whereTheLast(build().isStillUpdatingTheLog()).andThePrevious(
						build().finishedWith(SUCCESS)))));
	...
	public void should_describe_the_job_as_running_and_failing_if_it_is_running_and_the_previous_build_failed() {
		...
		List<JobView> views = asFollows(JobView.of(a(job().whereTheLast(build().hasntStartedYet()).andThePrevious(
				build().finishedWith(FAILURE)))),

		JobView.of(a(job().whereTheLast(build().isStillBuilding()).andThePrevious(build().finishedWith(FAILURE)))),
	...
	public void should_describe_the_job_as_successful_when_there_are_several_builds_running_in_parallel_and_the_last_completed_was_successful() {
		...
		view = JobView.of(a(job().whereTheLast(build().isStillBuilding()).andThePrevious(build().isStillBuilding())
	...
	public void should_describe_the_job_as_claimed_if_someone_claimed_last_build_failures() {
		...
		view = JobView.of(
				a(job().whereTheLast(
						build().finishedWith(FAILURE).andWasClaimedBy("Adam", "sorry, I broke it, fixing now"))),
	...
	public void should_know_who_has_been_committing_over_broken_build() {
		...
		view = JobView.of(a(job().whereTheLast(build().wasBrokenBy("Adam"))
				.andThePrevious(build().wasBrokenBy("Ben", "Connor")).andThePrevious(build().wasBrokenBy("Daniel"))
	...
	public void should_not_mention_any_culprits_if_the_build_was_successful_and_is_still_running() {
		...
		view = JobView.of(a(job().whereTheLast(build().isStillBuilding()).andThePrevious(
				build().succeededThanksTo("Adam"))));
	...
	public void should_indicate_culprits_if_the_build_is_failing_and_not_claimed() {
		...
		view = JobView.of(a(job().whereTheLast(build().wasBrokenBy("Adam"))), augmentedWith(Claim.class));
	...
	public void should_know_if_a_failing_build_has_been_claimed() throws Exception {
		...
		String ourPotentialHero = "Adam", theReason = "I broke it, sorry, fixing now";
	...
	public void should_describe_known_failures() {
		...
		view = JobView.of(a(job().whereTheLast(build().finishedWith(FAILURE).andKnownFailures(rogueAi))),
