...
describe('buildMonitor', function () {
    describe('stats', function () {
        ...
        describe('with access to the Internet', function() {

            beforeEach(inject(function($window) {
                $window.ga = gaStub;
            }));

            it ('does not flush the metrics if there are none recorded', inject(function($timeout, $window, googleAnalyticsBackend) {
                googleAnalyticsBackend.flush = sandbox.spy();

                $timeout.verifyNoPendingTasks();
            }));

            it ('flushes the timers after a "flushInterval" seconds', inject(function($timeout, $window, stats, googleAnalyticsBackend) {
                googleAnalyticsBackend.flush = sandbox.spy();
            ...
            it ('removes recorded metrics after the flush', inject(function($timeout, $window, stats, googleAnalyticsBackend) {
                googleAnalyticsBackend.flush = sandbox.spy();
            ...
            it ('starts fresh with every interval', inject(function($timeout, $window, stats, googleAnalyticsBackend) {
                googleAnalyticsBackend.flush = sandbox.spy();
            ...
            it ('accumulates metrics within an interval', inject(function($timeout, $window, stats, googleAnalyticsBackend) {
                googleAnalyticsBackend.flush = sandbox.spy();
            ...
            it ('accumulates metrics within a category', inject(function($timeout, $window, stats, googleAnalyticsBackend) {
                googleAnalyticsBackend.flush = sandbox.spy();
            ...
            it ('accumulates metrics per parameter', inject(function($timeout, $window, stats, googleAnalyticsBackend) {
                googleAnalyticsBackend.flush = sandbox.spy();
        ...
        describe ('no access to the Internet', function() {

            it ('does not gather any stats if it knows it won\'t be able to send them', inject(function($timeout, $window, stats) {
                $window.ga = gaNotLoaded();
