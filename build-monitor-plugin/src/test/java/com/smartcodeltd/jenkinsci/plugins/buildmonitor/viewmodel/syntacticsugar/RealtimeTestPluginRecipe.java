...
public class RealtimeTestPluginRecipe implements Supplier<PipelineRealtimeTestResultAction> {
    ...
    public RealtimeTestPluginRecipe withTestProgress(
            int expected, int completed, float expectedTime, float completedTime, String estimatedTime) {
        int completedPercent = Math.min((int) Math.floor((double) completed / (double) expected * 100d), 100);
        int completedTimePercent =
                Math.min((int) Math.floor((double) completedTime / (double) expectedTime * 100d), 100);
